...
!!! info
    The style guide is always a work in progress, and not all QuantumESPRESSORecipes code
    follows the rules. When modifying QuantumESPRESSORecipes, please fix the style violations
...
QuantumESPRESSORecipes uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/MineralsCloud/QuantumESPRESSORecipes.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the QuantumESPRESSORecipes directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to QuantumESPRESSORecipes have
